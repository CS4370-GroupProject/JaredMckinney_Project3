...
function setup() {
  ...
  tL = getNumber(tempLeft);
  tT = getNumber(tempTop);
  tR = tL + 100;
  tB = tT + 100;

  if (
    (tR == eL && tT == eT) // to the left of the empty block
    || (tB == eT && tL == eL) // to the top of the empty block
    || (tT == eT && tL == eR) // to the right of the empty block
    || (tT == eB && tL == eL) // to the bottom of the empty block
  )
  {
